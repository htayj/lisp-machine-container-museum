...
The rewritten, not-yet-run public factory now binds bootstrap independently to the
current deterministic M12 O2 build identity: 215,664 bytes and SHA-256
`42e1e7d37ac1b1cc3dabf5b22a38bc81702c1b1f45b6da8bf31f0ddb249a40e0`.
With the same four artifact identities, its derived artifact-set identity would be
`deddd6ff5bc626c1d62b354a28e757a638b6f824915df9b0c783fed5ebfc482a`.
These are source/build observations, not a replacement E26 runtime result; the E26
table and its older derived identity remain the exact identities of that retained
private-seam run.
...
| `M13-F06` | `E10` passed M10’s six-seam IndexedDB abort/terminate/reload campaign. `E24` loads the real `C-M10-IDB-v1` backend/controller and M4 worker-host bridge beside an independent selected-M12 state witness; it does not load the M13 shell or composite dispatch. Its synthetic-request stage failure leaves `CLEAN` with no replacement; its post-guest publication fault and lost host-complete response both fence `IN_DOUBT`, replace once, then recover `CLEAN` with an unchanged active durable receipt. `E25` moves that bridge into the shared M13 shell with a synthetic M4 peer: only a lower status-8 wait admits the sealed internal `host-next-request`/`host-complete` channel. The current source adds public v8 `base-media-mount`, tied to exact selected Wasm authority, a preceding successful bootstrap and selected import, the same storage binding, and a clean M10 reopen. Its synthetic fault matrix covers every lower status/read/adoption ordinal plus representative malformed/timeout/error/messageerror stages; its production binding test retains and revalidates every one of 258 indexed ranges, including the final 77-block range. E26 remains pre-change private-seam evidence: it reached a real first no-change write, but does not prove the new public path. | Partial — source public-mount contract, earlier selected no-change witness, and synthetic dispatch/lower failure algebra | Run exact selected-base changed-write/read/reopen, export/restore, and complete failure algebra through the new public shell path, including pre-completion retry and post-completion reread. |
...
| M13 composition | The shell has a bounded M10 dispatch seam: an injected real controller and bridge factory can service only lower status-8 guest waits through a sealed internal channel. The tracked source now also defines public v8 `base-media-mount`, which must follow selected import and bootstrap and precede clean M10 reopen; synthetic tests cover its range revalidation and mutation-discard matrix. E26 is a pre-change private selected-media run to tick 1,029,735 with one base-identical block-1 write and a clean no-change reopen/read; it cannot authenticate the current public path. | The public source contract is not a selected-runtime result. Changed-overlay persistence, full selected-base failure algebra, M8–M12/M10 application, lifecycle, accessibility, and audio workflows remain unproven. |
