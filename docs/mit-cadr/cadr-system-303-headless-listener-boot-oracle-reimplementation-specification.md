...
The block service never publishes the acknowledgement collection by itself.
It enforces generation `1`, a global completed-host-request high-water that
includes reads, a 1,024-total-host-transaction bound, and pre-success plus
post-completion target rereads. The service performs both reads and retains the
successful post-completion bytes and media state as an ordered witness snapshot.
After the matching writes complete, the M6 fast driver serializes their adjacent
issue/completion events into the exact `CDRM6HS1` transcript and consumes those
stored witness snapshots; M6 does not perform a third target read. It only then
constructs the collection. It binds both record ordinals and digests, all request identity and
...
commit page hash. A missing, duplicate, or forged arm pair, malformed ordering,
failed reread, drift, acknowledgement-member replay, reset/detach, fault,
completion failure, or
mismatch poisons the stream and aborts publication. M6 emits acknowledgement
schema v4 inside the ordered `IDENTITY_ACK_STREAM` schema v1 collection,
retaining optional immediately preceding read provenance. This is unit-tested M7 control-plane
support, not a frozen M6 release result or a new P4 runtime observation.

The exact committed request-1 LBA-1 descriptor-and-payload replay remains the
sole replay exception and retains precedence even after streaming begins. It is
the original M4 idempotent replay, creates no stream member, and does not weaken
the prohibition on replaying requests `135` and later acknowledgements.
