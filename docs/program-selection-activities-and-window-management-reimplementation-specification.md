...
- C303 chooses a type, then saves global `MOUSE-SHEET`. A cons-valued type expression
  ...
  select. Cleanup restores the saved global sheet. At either corner, a
  Middle-containing mask aborts, Left places normally, and a remaining
  Right-containing mask uses the common smart-point helper. Its complete recursive
  traversal, candidate-overwrite, and inclusive-clamp contract is normatively
  incorporated from [D03 Screen Editor and Frame-Up](screen-editor-and-frame-up-layout-design-reimplementation-specification.md#reshape-staged-trees)
  and applies independently at both Create corners.
