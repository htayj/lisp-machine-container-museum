...
| `debugger-dynamic-choices.png` | Shows the ordinary Debugger for a researcher-created condition, its selected frame, two condition-specific restart choices before any proceed operation, and the surrounding Listener's ragged top border with straight side edges | Publish beside the condition-system and Debugger analysis or the source-grounded Dynamic Windows ragged-border analysis; do not reuse as a generic error or Genera illustration |
