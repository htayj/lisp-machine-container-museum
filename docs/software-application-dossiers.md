...
| **D04 — Emergency Break and degraded interaction paths** | `Emergency Break`; cold-load-stream evaluator | `Emergency Break`; emergency error-recovery path | [Emergency Break and the cold-load stream](emergency-break-and-cold-load-stream.md) supplies the historical dossier. The [D04 reimplementation specification](emergency-break-and-degraded-interaction-paths-reimplementation-specification.md) defines release-selectable entry, editor, breakpoint, exact debugger-transition, ownership, recursive-degradation, failure, visual, and conformance contracts, including complete D04-owned source-bounded effective input trees. | `deep/existing; grouped` — public CADR/LM-3 and licensed Genera source ground the profiles; System 303 and Genera have reviewed runtime captures, and the Genera run verifies the actual System Menu leaf, separate cold client, `End`, result `42`, `Resume`, and restored main display. System 46 runtime and unexercised Function Suspend, cold-editor, query/Output-Hold, and automatic/nested fallback branches remain D04 oracle gaps; separate full Lisp Debugger and VLM Debugger application closure is routed to D12 and D52. |
| **D05 — EINE, ZWEI, and Zmacs editor family** | `EINE`; `ZWEI`; `Zmacs`; ZDT and ZTOP architecture; editor modes and command tables | `ZWEI`; `Zmacs`/`Editor`; editor modes, command tables, Teach Zmacs integration | [Cross-system lineage](lisp-machine-text-editors.md), [EINE](mit-cadr/eine.md), [CADR ZWEI/Zmacs](mit-cadr/zwei-and-zmacs.md), [Genera Zmacs](genera/zmacs.md), their binding references, and the [D05 reimplementation specification](eine-zwei-and-zmacs-editor-family-reimplementation-specification.md) provide release-selectable data, command-loop, complete source-profile binding-tree, file, mode, lifecycle, failure, visual, and conformance contracts. | `deep/existing; grouped; runtime-blocked` — EINE lacks a compatible band, System 46 lacks its declared Zmacs source/QFASL, and Genera's installed optional/site/user overlay graph remains a precise ignored-output oracle; the fixed selected-source profiles and representative System 303/Genera behavior are specified without filling those gaps by inference. |
| **D06 — Directory, difference, and buffer editors** | `Dired`; `BDired`; `Edit Buffers`; List Buffers; Kill Or Save Buffers | `Dired`; `Edit Buffers`; List Buffers; Kill Or Save Buffers; Command Processor `Compare Directories` | [Directory, difference, and buffer editors](directory-difference-and-buffer-editors.md) gives the explanatory architecture and evidence narrative. The [D06 reimplementation specification](directory-difference-and-buffer-editors-reimplementation-specification.md) defines release-selectable semantic views, complete effective keyboard/prefix/pointer/presentation/chooser trees, exact transaction and partial-failure order, source/manual defects, reviewed Edit Buffers visuals, protocol surfaces, and conformance tests. | `deep/existing; grouped; runtime-blocked` — System 46 lacks its declared Zmacs body; populated Dired/BDired, row-hit, chooser, and Compare Directories runtime oracles require disposable guest-visible files or synthetic buffers. The specified source profiles preserve the applications' distinct ZWEI/TV/Dynamic Windows substrates without filling those gaps by inference. |
| **D07 — Help, self-documentation, and Document Examiner** | ZWEI self-documentation; System Help; Lisp/flavor documentation; application Help handlers | `Document Examiner`; `doc-ex`; `small-doc-ex`; `Help Program`; `help`; `help-program`; Extended Help; `Documentation` command area | [Help, self-documentation, and Document Examiner](help-self-documentation-and-document-examiner.md) gives the evidence narrative. The [D07 reimplementation specification](help-self-documentation-and-document-examiner-reimplementation-specification.md) defines the release-selectable dispatcher, complete effective keyboard/prefix/menu/pointer/presentation trees, Lisp/flavor precedence, Help caches, DEX viewer/search/rendering/private-file/queue/hardcopy semantics, source defects, reviewed visuals, and conformance suite. | `deep/existing; grouped; runtime-blocked` — System 46, C303 Terminal/defect paths, Genera pop-up Help, Small DEX, optional overlays, and private-file/printer failure paths remain named runtime oracles; source semantics and rights-safe visual evidence are closed without merging public CADR and licensed Genera materials. |
...
| **D52 — Ivory, FEP, and Open Genera/VLM implementation layers** | CADR comparison only | `I-ARCHITECTURE`; `I-LINKER`; `I-LISP-COMPILER`; FEP file system; bus access; netboot; keyboard; embedding; compression/encryption; VLM host relationship | [Ivory, FEP, and Open Genera VLM implementation layers](ivory-fep-and-open-genera-vlm-implementation-layers.md) provides the complete physical FEP, IFEP/VLM Debugger, host configuration, architecture, platform-condition, safety, provenance, and runtime audit. | `deep/new; grouped; runtime-blocked` — the pause path and implementation systems were verified, and the harness can now target the Cold Load client exactly; the read-only VLM Debugger command run and capture-specific screenshot review remain unperformed. |
