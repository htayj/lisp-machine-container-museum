...
function error(id, code, message) {
  send({ type: "cadr-error", version: CADR_M3_PROTOCOL_VERSION, id, code, message });
}

function response(id, op, status, extra = {}, transfers = []) {
  send({
    type: "cadr-response",
    version: CADR_M3_PROTOCOL_VERSION,
    id,
    op,
    status: status >>> 0,
...
async function handle(request) {
  ...
  if (op === "input") {
    ...
    const view = new DataView(e.memory.buffer, output, 40);
    ...
    response(id, op, CADR_STATUS_OK, {
      request: { operation, generation, requestId, descriptorByteCount, completionByteCount },
...
async function receive(event) {
  ...
  if (!isRecord(request) || request.version !== CADR_M3_PROTOCOL_VERSION ||
